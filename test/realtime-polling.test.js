...
test('filtro WhatsApp separa oficiais, externas, ausência e REMOVE vigente', () => {
  ...
  assert.match(dbSource, /WA2_ANY_COMPLEMENTARY_LABEL_FILTER = '__complementary__'/);
  assert.match(dbSource, /WA2_NO_COMPLEMENTARY_LABEL_FILTER = '__none_complementary__'/);
  assert.match(dbSource, /current_wa2_labels/);
  assert.match(dbSource, /binding\.enabled = true/);
  assert.match(dbSource, /DISTINCT ON/);
  assert.match(dbSource, /parseWa2LabelKey/);
  ...
  assert.match(views, /Com qualquer etiqueta WhatsApp/);
  assert.match(views, /Sem nenhuma etiqueta WhatsApp/);
  assert.match(views, /Com qualquer etiqueta complementar/);
  assert.match(views, /Sem etiquetas complementares/);
  assert.match(views, /instance_id.*remote_label_id/);
  assert.match(views, /Etapas CRM/);
  assert.match(views, /Etiquetas complementares/);
  assert.match(dbSource, /remote_label_name/);
  assert.match(fs.readFileSync(new URL('../sql/008_wa2_label_catalog.sql', import.meta.url), 'utf8'), /ADD COLUMN IF NOT EXISTS remote_label_name/);
  const server = fs.readFileSync(new URL('../src/server.js', import.meta.url), 'utf8');
  const dashboard = server.slice(server.indexOf("app.get('/', async"), server.indexOf("app.get('/leads/export.csv'"));
  assert.doesNotMatch(dashboard, /listWa2Labels\(instance\.remote_instance_id\)/);
});
