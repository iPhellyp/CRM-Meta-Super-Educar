...
test('queries de vínculo mantêm tenant e não alteram funil ou dados Meta', () => {
  ...
  assert.match(linkQueries, /unlinked_at = now\(\)/);
  assert.doesNotMatch(linkQueries, /UPDATE leads SET stage/);
  assert.doesNotMatch(linkQueries, /meta_conversion_events/);
  assert.doesNotMatch(linkQueries, /\bDELETE\s+FROM\b/);
});
